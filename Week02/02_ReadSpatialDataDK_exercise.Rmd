...
Sometimes your spatial data will come with attributes such as area, length, etc. Unless you have just created these yourself, you cannot be sure of their provenance and accuracy. They may have also changed in the course of your spatial transformations. If you want to know what is the actual area of your polygons, then you can extract it directly from the geometries. 
...
# Can you plot the big_forests with their names??
...
The units are raster cells, and maybe a different unit of measurement was used to create the images?
...
plotRGB(
  aarhus, 
  stretch= "lin"); #lin makes it bright
plotRGB(
  aarhus, 
  add = TRUE)  #plotting on top of each other makes no sense
plotRGB(
  aarhus, 
  stretch= "lin") #this version is better
...
playgrounds_crs <- st_transform(
  playgrounds, 
  crs = the_crs)
# Project elevation to match the CRS of aarhus
elevation_crs <- projectRaster(
  elevation, 
  crs = the_crs, 
  method = "ngb")
...
arhus25832 <- projectRaster(
  aarhus, 
  crs = 25832,  
  method = "ngb")
st_crs(arhus25832) #should be the same as the playground object
...
plot(
  playgrounds_crs, 
  add = TRUE) #unable to make this work
